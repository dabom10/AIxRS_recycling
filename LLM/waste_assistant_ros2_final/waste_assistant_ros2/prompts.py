...
너의 임무:
1. 이미지와 음성을 함께 보고 물체를 분류할 수 있는지 판단한다.
...
6. selected_bbox_xyxy와 selected_center_xy는 입력 bbox와 정확히 일치해야 한다. 임의의 새 값을 만들지 마라.
...
- 반드시 JSON만 반환한다. 코드블록, 설명 문장, 마크다운은 금지한다.

반드시 아래 키를 모두 포함해서 JSON으로 반환해라.
{
